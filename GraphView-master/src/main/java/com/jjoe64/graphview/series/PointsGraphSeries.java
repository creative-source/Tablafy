...
public class PointsGraphSeries<E extends DataPointInterface> extends BaseSeries<E> {
     ...
     */
    private CustomShape mCustomShape [] = new CustomShape[8000];
    ...
    public void appendDataCustom(E dataPoint, boolean scrollToEnd, int maxDataPoints, boolean silent, CustomShape shape) {
        int xCoor = (int) dataPoint.getX();
        mCustomShape [xCoor] = shape;
        ...
        while (values.hasNext()) {
            ...
            if (!overdraw) {
                if (mCustomShape != null) {
                    mCustomShape [(int) value.getX()].draw(canvas, mPaint, endX, endY, value);
                    int xCoor = (int) value.getX();
                    Log.e("<DEBUG>","The value of endX is "+ xCoor);
